...
:- ensure_loaded(['miconicRevised.pl']).
...
/*-------The only fluent that is true initially ------------*/
lift_at(f0,[ ]).

/*----The goal state is defined with the list of fluents that must become true----*/
...
%   goalState( [served(p0), served(p1), served(p2)] ).  %needs at least 11 actions

goalState( [served(p0), served(p1)] ).  %can be solved with 8 actions
%  goalState( [served(p0)] ).  %can be solved with 4 actions
