...
%% The following line from the original Prolog file is commented out
%% :- module(pq,[insertPQ/4,removePQ/4,sizePQ/2,emptyPQ/1,pq_to_list/2,list_to_PQ/2,min_of_PQ/5,min_of_PQ/3,heapsort/2 ]).
...
% By the priority queue invariants, the root element always has a
...
% dont do any subtree swapping.  Finally, if Val is smaller than either
